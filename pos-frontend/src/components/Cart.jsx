...
function Cart({ cart, onUpdateQuantity, onRemoveItem, onCheckout, loading }) {
  ...
  useEffect(() => {
    ...
    if (customerType !== "royal" && cart.length > 0) {
      ...
      setPaymentAmount(total.toFixed(0));
    }
  ...
  const handleCheckout = () => {
    ...
    if (customerType === "online") {
      if (!customerName.trim() || !customerPhone.trim()) {
        alert(
          "Please enter customer name and phone number for Online customer",
        );
        return;
      }
    }
    ...
    if (customerType !== "royal") {
      if (payment < totalAfterDiscount) {
        alert(
          `Walk-In and Online customers must pay in full. Total: $${totalAfterDiscount.toFixed(2)}`,
        );
        return;
      }
    }
            ...
            <div className="customer-info-box">
              <div className="info-row">
                ...
                <span className="debit-amount">
                  {selectedCustomer.currentDebit.toLocaleString()} MMK
                </span>
              </div>
      ...
      {cart.length === 0 ? (
        ...
        <>
          ...
          <div className="payment-section-cart">
            ...
            {payment > 0 && (
              <div
                className={`balance-box ${balance >= 0 ? "positive" : "negative"}`}
              >
            ...
            {customerType === "royal" && selectedCustomer && balance < 0 && (
              <div className="credit-info-box">
                <p className="info-text">
                  Amount on credit: {Math.abs(balance).toLocaleString()} MMK
                </p>
                <p className="info-text">
                  New debit:{" "}
                  {(
                    selectedCustomer.currentDebit + Math.abs(balance)
                  ).toLocaleString()}{" "}
                  MMK
                </p>
            ...
            {(customerType === "walkin" || customerType === "online") &&
              payment > 0 &&
              balance < 0 && (
                <div className="payment-warning">
                  ⚠️ {customerType === "walkin" ? "Walk-In" : "Online"}{" "}
                  customers must pay in full
                </div>
              )}
          </div>
          ...
          >
            {loading
              ? "Processing..."
              : `Complete Sale - MMK ${totalAfterDiscount.toLocaleString()}`}
          </button>
        </>
      )}
    </div>
